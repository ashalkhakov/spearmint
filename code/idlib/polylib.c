...
void FreeWinding (winding_t *w)
{
	if (*(unsigned *)w == 0xdeaddead)
		ii.Com_Error (ERR_FATAL, "FreeWinding: freed a freed winding");
	...
	for (i=0 ; i<w->numpoints ; i++)
	{
		...
		if (DotProduct(v1, v2) < 0.999)
		{
			if (nump >= MAX_POINTS_ON_WINDING)
				ii.Com_Error (ERR_DROP, "RemoveColinearPoints: MAX_POINTS_ON_WINDING");
...
// find the major axis
	...
	if (x==-1)
		ii.Com_Error (ERR_DROP, "BaseWindingForPlane: no axis found");
...
// determine sides for each point
	...
	*back = b = AllocWinding (maxpts);
	...
	if (f->numpoints > maxpts || b->numpoints > maxpts)
		ii.Com_Error (ERR_DROP, "ClipWinding: points exceeded estimate");
	if (f->numpoints > MAX_POINTS_ON_WINDING || b->numpoints > MAX_POINTS_ON_WINDING)
		ii.Com_Error (ERR_DROP, "ClipWinding: MAX_POINTS_ON_WINDING");
}
...
void CheckWinding (winding_t *w)
{
	...
	if (w->numpoints < 3)
		ii.Com_Error (ERR_DROP, "CheckWinding: %i points",w->numpoints);
	
	area = WindingArea(w);
	if (area < 1)
		ii.Com_Error (ERR_DROP, "CheckWinding: %f area", area);
	...
	for (i=0 ; i<w->numpoints ; i++)
	{
		...
		for (j=0 ; j<3 ; j++)
			if (p1[j] > MAX_MAP_BOUNDS || p1[j] < -MAX_MAP_BOUNDS)
				ii.Com_Error (ERR_DROP, "CheckWinding: BUGUS_RANGE: %f",p1[j]);
	...
	// check the point is on the face plane
		...
		if (d < -ON_EPSILON || d > ON_EPSILON)
			ii.Com_Error (ERR_DROP, "CheckWinding: point off plane");
	...
	// check the edge isn't degenerate
		...
		if (VectorLength (dir) < ON_EPSILON)
			ii.Com_Error (ERR_DROP, "CheckWinding: degenerate edge");
	...
	// all other points must be on front side
		for (j=0 ; j<w->numpoints ; j++)
		{
			...
			if (d > edgedist)
				ii.Com_Error (ERR_DROP, "CheckWinding: non-convex");
		}
	}
}
