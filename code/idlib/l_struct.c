...
#include "q_shared.h"
#include "idlib_local.h"				//for the include of be_interface.h
#include "l_script.h"
#include "l_precomp.h"
#include "l_struct.h"
#include "l_utils.h"
...
int WriteStructWithIndent(fileHandle_t fp, structdef_t *def, char *structure, int indent)
{
	...
	for (i = 0; def->fields[i].name; i++)
	{
		fd = &def->fields[i];
		if (!WriteIndent(fp, indent)) return qfalse;
		FS_WriteFloatString( fp, "%s\t", fd->name );
		p = (void *)(structure + fd->offset);
		if (fd->type & FT_ARRAY)
		{
			num = fd->maxarray;
		...
		while(num-- > 0)
		{
			switch(fd->type & FT_TYPE)
			{
				case FT_CHAR:
				{
					FS_WriteFloatString( fp, "%d", *(char *) p );
					p = (char *) p + sizeof(char);
					break;
				} //end case
				case FT_INT:
				{
					FS_WriteFloatString( fp, "%d", *(int *)p );
					p = (char *) p + sizeof(int);
					break;
				} //end case
				case FT_FLOAT:
				...
				case FT_STRING:
				{
					FS_WriteFloatString( fp, "\"%s\"", (char *) p );
					p = (char *) p + MAX_STRINGFIELD;
					break;
				} //end case
				case FT_STRUCT:
