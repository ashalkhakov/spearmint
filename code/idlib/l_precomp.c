...
void QDECL SourceError(source_t *source, char *str, ...)
{
	...
	va_end(ap);
	ii.Com_Printf( S_COLOR_RED "Error: file %s, line %d: %s\n", source->scriptstack->filename, source->scriptstack->line, text );
...
void QDECL SourceWarning(source_t *source, char *str, ...)
{
	...
	Q_vsnprintf(text, sizeof(text), str, ap);
	va_end(ap);
	ii.Com_Printf(S_COLOR_YELLOW "Warning: file %s, line %d: %s\n", source->scriptstack->filename, source->scriptstack->line, text);
...
void PC_PushIndent(source_t *source, int type, int skip)
{
	...
	indent = (indent_t *) ii.GetMemory(sizeof(indent_t));
	indent->type = type;
...
//	t = (token_t *) malloc(sizeof(token_t));
	t = (token_t *) ii.GetMemory(sizeof(token_t));
//	t = freetokens;
	if (!t)
	{
		ii.Com_Error(ERR_FATAL, "out of token space");
		return NULL;
	} //end if
...
void PC_FreeDefine(define_t *define)
{
	...
	//free the define
	ii.FreeMemory(define->name);
	ii.FreeMemory(define);
...
//		{ "__STDC__", BUILTIN_STDC },
	...
	{
		define = (define_t *) ii.GetMemory(sizeof(define_t));
		Com_Memset(define, 0, sizeof(define_t));
		define->name = (char *) ii.GetMemory(strlen(builtin[i].string) + 1);
...
int PC_Directive_define(source_t *source)
{
	...
	define = (define_t *) ii.GetMemory(sizeof(define_t));
	Com_Memset(define, 0, sizeof(define_t));
	define->name = (char *) ii.GetMemory(strlen(token.string) + 1);
...
#if DEFINEHASHING
	src.definehash = ii.GetMemory(DEFINEHASHSIZE * sizeof(define_t *));
...
define_t *PC_CopyDefine(source_t *source, const define_t *define)
{
	...
	newdefine = (define_t *) ii.GetMemory(sizeof(define_t));
	//copy the define name
	newdefine->name = (char *) ii.GetMemory(strlen(define->name) + 1);
...
int PC_ReadToken(source_t *source, token_t *token)
{
	...
	while(1)
	{
		...
		if ( source->scriptstack->flags & SCFL_HANDLE_COMPATIBILITY ) {
			if (token->type == TT_STRING)
				StripDoubleQuotes(token->string);
...
source_t *LoadSourceFile(const char *filename, const define_t *globaldefines)
{
	...
	source = (source_t *) ii.GetMemory(sizeof(source_t));
...
#if DEFINEHASHING
	source->definehash = ii.GetMemory(DEFINEHASHSIZE * sizeof(define_t *));
...
#if DEFINEHASHING
	//
	if (source->definehash) ii.FreeMemory(source->definehash);
#endif //DEFINEHASHING
	//free the source itself
	ii.FreeMemory(source);
...
source_t *PC_LoadSource(const char *filename, const char *basepath, const define_t *globaldefines)
{
	int src = PC_LoadSourceHandle(filename, basepath, globaldefines);
	if (!src) {
		return NULL;
	}
	return sourceFiles[src];
} //end of the function PC_LoadSource
...
int PC_FreeSource(source_t *source) {
	int i;
	...
	for (i = 0; i < MAX_SOURCEFILES; i++) {
		if (sourceFiles[i] == source) {
			break;
		}
	}
	if (i == MAX_SOURCEFILES) {
...
void PC_CheckOpenSourceHandles(void)
{
	...
	for (i = 1; i < MAX_SOURCEFILES; i++)
	{
		...
		{
#ifdef BOTLIB
			ii.Com_Printf(S_COLOR_RED "Error: file %s still open in precompiler\n", sourceFiles[i]->scriptstack->filename);
#endif	//BOTLIB
		} //end if
	} //end for
...
================
*/
int PC_ParseInt( source_t *src ) {
	token_t token;

	if ( !PC_ReadToken( src, &token ) ) {
		SourceError( src, "couldn't read expected integer" );
		return 0;
	}
	if ( token.type == TT_PUNCTUATION && !strcmp( token.string, "-" ) ) {
		PC_ExpectTokenType( src, TT_NUMBER, TT_INTEGER, &token );
		return -((signed int) token.intvalue);
	}
	else if ( token.type != TT_NUMBER || token.subtype == TT_FLOAT ) {
		SourceError( src, "expected integer value, found '%s'", token.string );
	}
...
================
*/
qboolean PC_ParseBool( source_t *src ) {
	token_t token;

	if ( !PC_ExpectTokenType( src, TT_NUMBER, 0, &token ) ) {
		SourceError( src, "couldn't read expected boolean" );
		return qfalse;
	}
...
================
*/
float PC_ParseFloat( source_t *src, qboolean *errorFlag ) {
	token_t token;
	...
	if ( !PC_ReadToken( src, &token ) ) {
		if ( errorFlag ) {
			SourceWarning( src, "couldn't read expected floating point number" );
			*errorFlag = qtrue;
		} else {
			SourceError( src, "couldn't read expected floating point number" );
		}
		return 0;
	}
	if ( token.type == TT_PUNCTUATION && !strcmp( token.string, "-" ) ) {
		PC_ExpectTokenType( src, TT_NUMBER, 0, &token );
		return -token.floatvalue;
	}
	else if ( token.type != TT_NUMBER ) {
		if ( errorFlag ) {
			SourceWarning( src, "expected float value, found '%s'", token.string );
			*errorFlag = qtrue;
		} else {
			SourceError( src, "expected float value, found '%s'", token.string );
		}
	}
	return token.floatvalue;
}
...
================
*/
int PC_Parse1DMatrix( source_t *src, int x, float *m ) {
	int i;
...
================
*/
int PC_Parse2DMatrix( source_t *src, int y, int x, float *m ) {
	int i;
	...
	for ( i = 0; i < y; i++ ) {
		if ( !PC_Parse1DMatrix( src, x, m + i * x ) ) {
			return qfalse;
		}
	}
...
================
*/
int PC_Parse3DMatrix( source_t *src, int z, int y, int x, float *m ) {
	int i;
	...
	for ( i = 0 ; i < z; i++ ) {
		if ( PC_Parse2DMatrix( src, y, x, m + i * x*y ) ) {
			return qfalse;
		}
	}
...
================
*/
int PC_ReadTokenOnLine( source_t *src, token_t *token ) {
	if ( !PC_ReadToken( src, token ) ) {
...
=================
*/
int PC_SkipBracedSection( source_t *src, qboolean parseFirstBrace ) {
	token_t token;
	int depth;
