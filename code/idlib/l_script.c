...
void PS_CreatePunctuationTable(script_t *script, punctuation_t *punctuations)
{
	...
	if (!script->punctuationtable) script->punctuationtable = (punctuation_t **)
												ii.GetMemory(256 * sizeof(punctuation_t *));
...
script_t *LoadScriptFile(const char *filename)
{
	...
	buffer = ii.GetMemory(sizeof(script_t) + length + 1);
...
{
#ifdef PUNCTABLE
	if (script->punctuationtable) ii.FreeMemory(script->punctuationtable);
#endif //PUNCTABLE
	ii.FreeMemory(script);
