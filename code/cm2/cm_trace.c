...
void CM_TraceTrmThroughNode( cm_traceWork_t *tw, cm_node_t *node ) {
	...
	if ( tw->positionTest ) {
		...
		for ( bref = node->brushes; bref; bref = bref->next ) {
			if ( CM_TestTrmVertsInBrush( tw, bref->b ) ) {
				return;
			}
		}
		...
		for ( pref = node->polygons; pref; pref = pref->next ) {
			if ( CM_TestTrmInPolygon( tw, pref->p ) ) {
				return;
			}
		}
	}
		...
		for ( pref = node->polygons; pref; pref = pref->next ) {
			if ( CM_RotateTrmThroughPolygon( tw, pref->p ) ) {
				return;
			}
		}
	}
		...
		for ( pref = node->polygons; pref; pref = pref->next ) {
			if ( CM_TranslateTrmThroughPolygon( tw, pref->p ) ) {
				return;
			}
		}
	}
...
void CM_TraceThroughAxialBSPTree_r( cm_traceWork_t *tw, cm_node_t *node, float p1f, float p2f, vec3_t p1, vec3_t p2) {
	...
	if ( node->polygons || (tw->positionTest && node->brushes) ) {
		...
		CM_TraceTrmThroughNode( tw, node );
	}
	// if already stuck in solid
	...
	CM_TraceThroughAxialBSPTree_r( tw, node->children[0], p1f, p2f, p1, p2 );
	CM_TraceThroughAxialBSPTree_r( tw, node->children[1], p1f, p2f, p1, p2 );
	return;
#endif
	...
	CM_TraceThroughAxialBSPTree_r( tw, node->children[side], p1f, midf, p1, mid );
	...
	CM_TraceThroughAxialBSPTree_r( tw, node->children[side^1], midf, p2f, mid, p2 );
}

/*
================
CM_TraceThroughModel
================
*/
void CM_TraceThroughModel( cm_traceWork_t *tw ) {
	...
	if ( !tw->rotation ) {
		...
		CM_TraceThroughAxialBSPTree_r( tw, tw->model->node, 0, 1, tw->start, tw->end );
	}
	else {
		...
		if ( d > CIRCLE_APPROXIMATION_LENGTH ) {
            ...
            VectorCopy( tw->start, start );
			// trace circle approximation steps through the BSP tree
			for ( i = 0; i < numSteps; i++ ) {
				// calculate next point on approximated circle
                RotationFromOriginAndVector( &rot, tw->origin, tw->axis, tw->angle * ((float) (i+1) / numSteps) );
                MatrixRotateVector( start, rot.axis, end );
				// trace through spatial subdivision and then through leafs
				CM_TraceThroughAxialBSPTree_r( tw, tw->model->node, 0, 1, start, end );
		...
		CM_TraceThroughAxialBSPTree_r( tw, tw->model->node, 0, 1, start, tw->end );
	}
}
